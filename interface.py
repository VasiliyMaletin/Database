...
def listbox_fill(pupil_dict):
    ...
    for i in range(len(pupil_dict)):
        pupil_id_list.append(pupil_dict[i]["id"])
        pupil_listbox.insert(END, pupil_dict[i]["ФИО"])
...
def window_init():
    ...
    pupil_listbox = Listbox(root, justify=LEFT, font = common_font)#, listvariable = list_var)
